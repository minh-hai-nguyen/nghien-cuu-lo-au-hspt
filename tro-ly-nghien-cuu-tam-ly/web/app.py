...
# =========================================================
# AUTHOR KG + RAG (keyword-based retrieval)
# =========================================================
import re, math
from collections import Counter as _Counter

_AUTHOR_DATA_DIR = Path(__file__).parent / 'data'
_AUTHOR_INDEX = None
_AUTHOR_KG = None
_AUTHOR_NORM = None
...
def _load_author_data():
    ...
    try:
        idx_path = _AUTHOR_DATA_DIR / 'rag_authors_index.json'
        kg_path = _AUTHOR_DATA_DIR / 'author_kg_v1.json'
        norm_path = _AUTHOR_DATA_DIR / 'authors_normalized.json'
        if idx_path.exists():
...
_AUTHOR_STOPWORDS = set('''a an and or but in on at to from for of with by is are was were be been being have has had do does did the this that these those it its they them their we us our you your he she his her i me my là và hoặc nhưng trong tại đến từ cho của với bởi là các một những này đó nó họ chúng ta chúng tôi tôi bạn anh ấy cô ấy có được bị đã đang sẽ cũng không chỉ rất đều như ai'''.split())

def _author_tokenize(text):
    if not text: return []
    text = text.lower()
    tokens = re.findall(r'\b[\wÀ-ỹđĐ]+\b', text, flags=re.UNICODE)
    return [t for t in tokens if t not in _AUTHOR_STOPWORDS and len(t) > 1]

def _author_score(query_tokens, entry, idf):
    """TF-IDF dot product + name match boost + priority boost."""
    etok = entry['tokens']
    score = 0.0
    for qt in query_tokens:
        if qt in etok:
            score += etok[qt] * idf.get(qt, 1.0)
    # Exact surname match boost
...
async def authors_query(request: Request):
    """Keyword search over author profiles."""
...
def _load_questions():
    ...
    try:
        qidx = _AUTHOR_DATA_DIR / 'rag_questions_index.json'
        qkg = _AUTHOR_DATA_DIR / 'questions_kg.json'
        if qidx.exists():
...
# =========================================================
# GLOSSARY (abbreviations + detailed terms + organizations)
# =========================================================
_GLOSSARY = None
...
def _load_glossary():
    global _GLOSSARY
    try:
        p = _AUTHOR_DATA_DIR / 'glossary.json'
        if p.exists():
            with open(p, encoding='utf-8') as f:
...
async def glossary_search(q: str = ''):
    """Search glossary — match against abbreviation keys, detailed term names, or organization names."""
    if not _GLOSSARY:
        return JSONResponse({'error': 'not loaded'}, status_code=500)
    q = q.strip()
    if not q:
    ...
    # Search abbreviations — case-insensitive exact or contains
